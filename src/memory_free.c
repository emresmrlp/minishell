...
/*   Updated: 2025/08/01 15:17:08 by makpolat         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	free_command_node(t_command *cmd)
{
	...
	if (cmd->input_fds)
		free_command_args(cmd->input_fds);
	if (cmd->output_fds)
		free_command_args(cmd->output_fds);
	if (cmd->append_fds)
		free_command_args(cmd->append_fds);
	if (cmd->heredoc_fds)
		free_command_args(cmd->heredoc_fds);
